...
<div class="card">
	<div class="card-header">
		<h3 class="card-title">Editar página {{$pagina->Nombre}}</h3>
	...
	<div class="card-body">
        ...
        @endif      
        <p class="text-muted">Creado por: {{$pagina->creator->email}} @if($pagina->creator_id != $pagina->editor_id) | Editado por: {{$pagina->editor->email}} @endif</p>
        <form method="post" action="{{route('admineditpagina')}}" class="editapagina">
            @csrf                   
            <input type="hidden" name="type" value="edit"/>
            <input type="hidden" name="id" value="{{$pagina->id}}"/>
            <div class="form-group">
                <label for="titulo"><span class="text-danger">*</span> Título</label>
                <input type="text" maxlength="9" class="form-control" id="titulo" name="titulo" value="{{$pagina->titulo}}" required>                                                   
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="descripcion" name="descripcion" value="{{$pagina->descripcion}}" required>
                <small class="text-muted">* Longitud máxima permitida 190 caracteres</small>
            </div>       

            <div class="form-group">
                <label for="link"><span class="text-danger">*</span> Link</label>
                <input type="url" class="form-control" id="link" name="link" value="{{$pagina->link}}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2" id="carpetas" name="carpetas[]" required multiple title="Selecciona una...">
                    @foreach($carpetasarray as $key => $carpeta)
                        <option value="{{$key}}" @if(in_array($key, json_decode($pagina->id_carpeta))) selected @endif>{{$carpeta}}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                <label for="posicion"><span class="text-danger">*</span> Posición</label>
                <input type="number" class="form-control" id="posicion" name="posicion" value="{{$pagina->position}}" min="1" max="100" required>
                <small class="text-muted">* orden de vista de ayudas, el orden es de 1 a 100</small>
            </div>
            <div class="form-group">                                             
                <input type="checkbox" name="activa" @if($pagina->activa == 1) checked @endif>
                <label for="activa">Activar</label>
                <small class="text-muted">* Marcado = activada, Desmarcado = desactivada</small>
            </div>                  
            <button type="submit" class="btn btn-primary btn-sm">Editar página</button>
        </form>
	</div>
