...
<div class="card">
	<div class="card-header">
		<h3 class="card-title">Usuarios sin validar</h3>
        ...
        <div class="table-responsive">
            <table class="table table-hover text-nowrap f-14" style="overflow-x:scroll" id="table3">
                ...
                <tbody>
                    @if($usuarios->count() > 0)
                        @foreach($usuarios as $user)
                            <tr>
                                ...
                                <td>
                                {{ html()->form('POST', route('validateuser'))->class('validateuser')->open()}}
                                    {{html()->hidden('id', $user->id) }}
                                    <button type="submit" class="btn btn-primary btn-xs"><i class="fa-solid fa-thumbs-up"></i> Validar</button>
                                {{ html()->form()->close()}}
                                </td>
                            </tr>
                        @endforeach
                    @else
                    <tr>
                        <td colspan="7">No hay usuarios sin validar</td>
                    </tr>
                    @endif
                </tbody>
            </table>
